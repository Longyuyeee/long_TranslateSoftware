...
6. Before replacing or deleting that executable, run `long-translate.exe --unregister-native-host`.

The popup should report the desktop version and pairing state. Translation requires an approved `translation` capability. Browser-internal pages and extension stores must reject script injection; the extension does not request persistent host permissions or declare always-on content scripts.
