...
function popupFixture(): void {
  document.body.innerHTML = `
    ...
    <button id="check"></button><button id="enable-selection"></button>
...
describe("browser popup active-tab boundary", () => {
  ...
  it("injects the reviewed content script only after an explicit click", async () => {
    ...
    vi.stubGlobal("chrome", {
      ...
      tabs: { query: vi.fn((_query, callback: (tabs: Array<{ id: number; url: string }>) => void) => callback([{ id: 42, url: "https://example.com/article" }])) },
    ...
    expect(document.getElementById("selection-status")?.textContent).toContain("已启用");
  });
  ...
  it("reports restricted browser pages without widening permissions", async () => {
    ...
    vi.stubGlobal("chrome", {
      runtime,
      tabs: { query: (_query: unknown, callback: (tabs: Array<{ id: number; url: string }>) => void) => callback([{ id: 7, url: "chrome://settings" }]) },
      scripting: { executeScript },
    });
    await import("./popup");
    document.getElementById("enable-selection")?.click();

    expect(document.getElementById("selection-status")?.textContent).toContain("无法访问");
    expect((document.getElementById("enable-selection") as HTMLButtonElement).disabled).toBe(false);
    expect(executeScript).not.toHaveBeenCalled();
  });
});
