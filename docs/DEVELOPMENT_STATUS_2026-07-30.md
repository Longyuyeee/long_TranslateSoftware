...
审计范围：`master` 已合并 PR #22～#30，以及当前生词本 Rust 边界分支
...
| 当前进展 | PR #22～#30 已合并；生词本命令、上下文与导出 Rust 边界正在独立分支推进 | 正常 |
| 前端测试 | 41 个文件 / 182 项 | 通过 |
| Rust 测试 | 64 项单元测试 + 2 项真实子进程测试 | 通过 |
...
- Rust 已拆出配置、快捷键、生命周期、系统集成、词库命令/查询/导出、历史、复习、备份、诊断、TTS、OCR、WebDAV 等领域模块。
...
| `src-tauri/src/lib.rs` | 约 328 行、8 个直接命令 | 生词本数据库查询、导出格式和重复旧同步实现已移除；剩余应用统计、OCR 编排与诊断入口 |
...
1. 将 `lib.rs` 的应用统计、OCR 编排与诊断入口迁入对应 Rust 模块。
...
每一步独立提交、独立 PR，并保持前端 182 项和 Rust 66 项既有测试不回退。
