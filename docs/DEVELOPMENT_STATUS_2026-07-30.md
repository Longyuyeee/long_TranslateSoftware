...
| 当前进展 | PR #22～#32 已合并；OCR 命令编排 Rust 边界正在独立分支推进 | 正常 |
| 前端测试 | 41 个文件 / 182 项 | 通过 |
| Rust 测试 | 67 项单元测试 + 2 项真实子进程测试 | 通过 |
...
- Rust 已拆出配置、快捷键、生命周期、系统集成、词库命令/查询/导出、应用统计、历史、复习、备份、诊断、TTS、OCR 命令/识别、WebDAV 等领域模块。
...
| `src-tauri/src/lib.rs` | 约 229 行、2 个直接命令 | OCR 编排已移除；剩余诊断导出、更新器状态与应用壳初始化 |
...
1. 将 `lib.rs` 的诊断导出与更新器状态入口迁入对应 Rust 模块。
...
每一步独立提交、独立 PR，并保持前端 182 项和 Rust 69 项既有测试不回退。
