...
| 当前进展 | PR #22～#28 已合并；TTS 路由与播放边界正在独立分支推进 | 正常 |
| 前端测试 | 41 个文件 / 181 项 | 通过 |
...
1. 拆分 `GeneralSettingsTab.tsx` 的 WebDAV、快捷键和维护区块。
2. 将 `lib.rs` 的词库/导出、同步和统计命令迁入对应 Rust 模块。
3. 建立统一 Tauri 错误结构，先覆盖新增路径，再迁移旧命令。

每一步独立提交、独立 PR，并保持前端 181 项和 Rust 64 项既有测试不回退。
