...
| 当前 PR | Draft PR #22，可合并，`quality` 成功 | 等待审阅/合并 |
| 前端测试 | 36 个文件 / 154 项 | 通过 |
...
| GitHub 任务 | 0 个开放 Issue；1 个开放 Draft PR | 路线尚未转为可跟踪任务 |
...
| 边界 | 当前 | 既定目标/问题 |
| --- | ---: | --- |
| `Dashboard.tsx` | 958 行 | 目标不超过 600 行；仍承担主界面跨域编排 |
| `api.ts` | 852 行 | 网络、翻译策略、流式任务和 TTS 仍集中 |
...
- 审阅 Draft PR #22 的 Schema、双端模型、限制和文档；
...
1. 把 `Dashboard.tsx` 的剩余跨域状态和面板编排下沉，控制到 600 行以内。
2. 将 `api.ts` 拆为传输层、翻译任务/故障切换和 TTS 路由，保留兼容出口。
3. 将 `lib.rs` 的词库/导出、同步和统计命令迁入对应 Rust 模块。
4. 建立统一 Tauri 错误结构，先覆盖新增路径，再迁移旧命令。
5. 拆分 `GeneralSettingsTab.tsx` 的 WebDAV、快捷键和维护区块。

每一步独立提交、独立 PR，并保持前端 154 项和 Rust 64 项既有测试不回退。
