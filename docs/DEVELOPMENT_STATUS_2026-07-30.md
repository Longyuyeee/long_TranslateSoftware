...
| 当前进展 | PR #22、#23 已合并；Dashboard 同步/事件边界正在独立分支推进 | 正常 |
| 前端测试 | 38 个文件 / 161 项 | 通过 |
...
| 边界 | 当前 | 既定目标/问题 |
| --- | ---: | --- |
| `Dashboard.tsx` | 717 行 | 导航外壳与同步/事件已拆分；目标不超过 600 行，仍承担数据操作编排 |
...
1. 主界面外壳及 WebDAV/统计/事件编排已拆分；继续把 `Dashboard.tsx` 的数据导入导出、生词本导出和维护动作下沉，控制到 600 行以内。
...
每一步独立提交、独立 PR，并保持前端 161 项和 Rust 64 项既有测试不回退。
