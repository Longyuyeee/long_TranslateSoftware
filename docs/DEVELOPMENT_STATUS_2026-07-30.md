...
审计范围：`master` 已合并 PR #22～#31，以及当前应用统计 Rust 边界分支
...
- Native Messaging v1 协议基线已经在 PR #22 中完成并合并，CI 通过；它是 `v0.5.0` 的设计输入，不代表 Native Host 或浏览器扩展已经可用。
...
| 当前进展 | PR #22～#31 已合并；应用统计与翻译计数 Rust 边界正在独立分支推进 | 正常 |
| 前端测试 | 41 个文件 / 182 项 | 通过 |
| Rust 测试 | 66 项单元测试 + 2 项真实子进程测试 | 通过 |
...
- Rust 已拆出配置、快捷键、生命周期、系统集成、词库命令/查询/导出、应用统计、历史、复习、备份、诊断、TTS、OCR、WebDAV 等领域模块。
...
| `api.ts` | 37 行 | 仅保留翻译、连接、质量辅助与 TTS 的兼容重导出 |
| `GeneralSettingsTab.tsx` | 212 行 | 快捷键、WebDAV 与维护区块已拆分，低于 400 行目标 |
| `src-tauri/src/lib.rs` | 约 277 行、6 个直接命令 | 生词本与应用统计逻辑已移除；剩余 OCR 编排、诊断入口与应用壳初始化 |
...
1. 将 `lib.rs` 的 OCR 编排与诊断入口迁入对应 Rust 模块。
2. 建立统一 Tauri 错误结构，先覆盖新增路径，再迁移旧命令。

生词本增删改查、完整上下文读取和 CSV/JSON 导出已经迁入 `wordbook.rs`；命令协议和完整列表语义保持不变，可空旧数据、CSV 转义与 JSON 结构已纳入测试。未注册且与 `webdav.rs` 重复的旧同步函数已经删除。

应用统计与翻译计数已经迁入 `app_stats.rs`；命令字段保持兼容，翻译计数改为 SQLite 原子自增，损坏计数和异常安装日期边界已纳入测试。

每一步独立提交、独立 PR，并保持前端 182 项和 Rust 68 项既有测试不回退。
