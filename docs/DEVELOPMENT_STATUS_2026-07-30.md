...
| 当前进展 | PR #22 已合并；Dashboard 外壳边界正在独立分支推进 | 正常 |
| 前端测试 | 37 个文件 / 157 项 | 通过 |
...
| 边界 | 当前 | 既定目标/问题 |
| --- | ---: | --- |
| `Dashboard.tsx` | 803 行 | 导航外壳已拆分；目标不超过 600 行，仍承担跨域编排 |
...
1. 主界面外壳已拆分；继续把 `Dashboard.tsx` 的 WebDAV、数据导入导出和统计/事件编排下沉，控制到 600 行以内。
...
每一步独立提交、独立 PR，并保持前端 157 项和 Rust 64 项既有测试不回退。
