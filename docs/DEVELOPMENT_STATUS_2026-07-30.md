...
> 本文是 2026-07-30 的发布前历史快照。`v0.4.9` 已正式发布；当前产品范围与执行顺序以 [`DEVELOPMENT_PLAN_2026-08-10.md`](DEVELOPMENT_PLAN_2026-08-10.md) 为准。

审计日期：2026-07-30

审计范围：`master` 已合并 PR #22～#35，以及当前 `v0.4.9` 候选发布分支
...
- `v0.4.9` 的架构与回归收口已经完成，三处源版本和两份锁文件进入候选状态；正式发布仍需签名安装包交互烟雾。
...
| 当前进展 | PR #22～#35 已合并；`v0.4.9` 版本与发布说明已进入候选分支 | 正常 |
| 前端测试 | 42 个文件 / 184 项 | 通过 |
| Rust 测试 | 70 项单元测试 + 2 项真实子进程测试 | 通过 |
...
- Rust 已拆出配置、快捷键、生命周期、系统集成、词库命令/查询/导出、应用统计、历史、复习、备份、诊断、更新器配置状态、TTS、OCR 命令/识别、WebDAV 等领域模块。
...
- `docs/releases/v0.4.9.md` 已创建，前端、Cargo、Tauri 与锁文件版本已经一致。
- 本地签名 NSIS、MSI 与两份 Updater `.sig` 已生成，SHA-256 已记录；真实 Windows 安装与升级交互烟雾尚待执行。
...
| `src-tauri/src/lib.rs` | 约 177 行、无直接命令 | 领域命令编排已移除，仅保留模块注册、迁移与应用壳初始化 |
| Tauri 错误 | 应用统计、OCR 与诊断已使用 `code + message`；旧命令仍有 `Result<_, String>` | 契约与前端兼容层已建立，旧命令按实际修改范围渐进迁移 |
...
1. 统一 Tauri 错误结构已建立，首批覆盖应用统计、OCR 与诊断；更新器状态保持无失败分支的布尔协议。
2. 三处版本和两份锁文件已提升至 `0.4.9`，版本化说明、签名候选安装包与 SHA-256 记录已经生成；下一步执行交互烟雾。
...
OCR Base64/截图识别、确认文本和语言枚举命令已经迁入 `ocr.rs`；命令协议、窗口动作和事件保持兼容，Data URL、配置读取、空文本、零尺寸及坐标溢出边界已纳入测试。

脱敏诊断导出已经迁入 `diagnostics.rs`，更新器公钥配置状态已经迁入独立 `updater.rs`；命令协议保持兼容，占位和空白公钥均按未配置处理，`lib.rs` 不再直接定义 Tauri 命令。

统一 `CommandError` 已定义稳定错误代码与消息；应用统计、OCR 和诊断命令完成首批迁移，前端解析器兼容结构化对象、旧字符串与 JavaScript `Error`，用户取消仍保持静默。

每一步独立提交、独立 PR，并保持前端 184 项和 Rust 72 项既有测试不回退。
