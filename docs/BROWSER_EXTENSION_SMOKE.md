...
状态：自动门禁与真实 Edge 运行时烟雾已完成；v0.5.0 的签名候选全量人工验收由项目负责人接受风险并跳过
...
npm run smoke:browser:runtime -- --require-desktop
```
...
v0.5.0 由项目负责人接受未执行签名候选全量人工验收的风险；后续商店上架或正式 ID 变更时必须重新执行本节，自动化测试和构建不能替代该门槛。
...
v0.5.0 由项目负责人接受未执行签名候选全量人工验收的风险；后续商店上架或正式 ID 变更时必须重新执行本节，自动化测试只证明协议和编排边界。
