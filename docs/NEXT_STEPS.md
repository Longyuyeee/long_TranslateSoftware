...
- PDF 导入现在会保守识别明显分离且纵向重叠的定位文本列，并以 `complex-layout-review-required` 给出逐页中英文提醒；4 份固定哈希的官方真实 PDF、Checkpoint round-trip 和双模式 DOCX 导出门禁已重新通过，三份已知复杂的一页语料均被逐页标记。该检测只提升风险可见性，不擅自改变内容流顺序，也不改变 PDF 预览能力边界。
...
对应远端分支为 `codex/development-roadmap-20260903`，已推送增量提交包括 `52eb795`、`8fdeea3`、`53acb58`、`aa4cbc8`、`ebbc934`、`4cecc60` 和 `5ff4524`；后续进展以该分支提交历史为准。

## 当前情况

- 当前稳定版为 `v0.5.3`，PR `#112` 已通过 CI 并合并至发布基线 `c8441f4`。Release workflow `32504462821` 的全量质量门禁、Windows 构建、Updater 签名和发布步骤全部通过，NSIS、MSI、两份签名、`latest.json`、浏览器扩展 ZIP 和质量报告共 7 个正式资产均已在线复核。
- `v0.5.3` 完成桌面体验收口：左侧导航按翻译/学习/设置重组，生词本工具栏与词条列表拆分，默认与最小窗口响应式修复，全局背景、玻璃卡片、间距和批量翻译断点统一；此前完成的主窗口真实版本显示也已随本版发布。
- v0.5.3 发布基线的 8 个主页面在 1000×700 与 820×560 下逐页无横向溢出，生词本另以 8 条完整词条验证双栏/横向列表和独立滚动。该发布基线的前端 62 个测试文件 / 328 项全部通过，最大桌面 chunk 256.33 KiB / 300 KiB，扩展 34.33 KiB / 64 KiB，npm 高危依赖 0 个；在线 `latest.json` 为 0.5.3，三个 Windows 平台条目均包含签名并可跳转至对应安装包。
- 上一个稳定版为 `v0.5.2`，标签指向发布基线 `7b900db`。Release workflow `32440046681` 的全量质量门禁、Windows 构建、Updater 签名和发布步骤全部通过，7 个正式资产及在线 `latest.json` 已复核。
...
- v0.5.2 发布门禁为前端 62 个测试文件 / 326 项、Rust 158 项单元测试和 11 项进程/注册集成测试，并显式通过 3 条真实 PDF 语料/Checkpoint/双模式导出测试；严格 Clippy 通过。全仓 `cargo fmt --check` 仍会被既有 `db.rs`、`ocr.rs` 格式差异阻断，本次没有夹带这些用户本地修改。
- 当前开发分支门禁为前端 62 个测试文件 / 329 项，最大桌面 JavaScript chunk 256.61 KiB / 300 KiB，浏览器扩展 34.33 KiB / 64 KiB；Browserslist 已完成安全更新，官方 npm 审计为 0 个漏洞。
...
1. v0.5.3 界面收口已经发布；下一阶段优先收集浏览器扩展与 PDF/Word 翻译的真实使用反馈，不扩展到无关功能。
2. 保留 `npm run audit:docx:word` 作为后续兼容性门槛；只有同批真实文档通过后，才允许移除 Word 未验证声明。
3. 为 PDF 增加更多真实多栏/复杂内容流语料，继续以预览顺序和成品差异驱动修正；扫描件 OCR 仍单独立项。
4. Authenticode、浏览器商店上架和全新 Windows VM 安装覆盖继续分别立项，不与常规功能增量混合。
