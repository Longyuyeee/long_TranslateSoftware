...
- `v0.5.3` 已完成桌面体验收口候选：左侧导航按翻译/学习/设置重组，生词本工具栏与词条列表拆分，默认与最小窗口响应式修复，全局背景、玻璃卡片、间距和批量翻译断点统一。发布完成后在此补记标签、工作流和正式资产复核结果。
...
- v0.5.3 界面候选包含此前尚未发布的主窗口版本显示；版本优先读取 Tauri 运行时，构建元数据只作失败回退。8 个主页面在 1000×700 与 820×560 下逐页无横向溢出，生词本另以 8 条完整词条验证双栏/横向列表和独立滚动。前端 62 个测试文件 / 328 项全部通过，最大桌面 chunk 256.33 KiB / 300 KiB，扩展 34.33 KiB / 64 KiB，npm 高危依赖 0 个。
...
1. v0.5.3 界面收口发布后，优先继续浏览器扩展与 PDF/Word 翻译的真实使用反馈，不扩展到无关功能。
