...
- 8 个主页面在 1000×700 与 820×560 下逐页无横向溢出，生词本另以 8 条完整词条验证双栏/横向列表和独立滚动。前端 62 个测试文件 / 328 项全部通过，最大桌面 chunk 256.33 KiB / 300 KiB，扩展 34.33 KiB / 64 KiB，npm 高危依赖 0 个；在线 `latest.json` 为 0.5.3，三个 Windows 平台条目均包含签名并可跳转至对应安装包。
