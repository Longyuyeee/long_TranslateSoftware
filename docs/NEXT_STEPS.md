...
- Native Messaging v1 已完成严格 Schema、Rust/TypeScript 模型、版本协商、配对状态、错误码、大小/并发限制和 32 位 Chromium 扩展 ID 精确校验；桌面 EXE 可直接进入最小 Host 模式，并已有 manifest 原子写入、Chrome/Edge HKCU 幂等注册/升级/可逆卸载和真实 Windows 注册表测试。固定开发 ID 的最小 Manifest V3 扩展已实现同端口 `hello` / `ping`，NSIS/WiX 已接入安装、升级、卸载与回滚；真实 Chrome/Edge 烟雾、商店 ID、私有 IPC 和翻译能力尚未完成。
...
2. 桌面私有 IPC 的 Windows 命名管道、随机端点、令牌校验和在线探针已经完成；Host 已能在强制 `hello` 首帧后把配对申请转入桌面并保持 `pending`。下一步实现桌面批准、拒绝与撤销的最小持久化和 UI，再接 `translate` / `cancel` / `add_word`。
