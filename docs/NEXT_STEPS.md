...
2. PDF 已作为独立后续版本启动：受限文本层解析、4 份真实公开语料和最小阅读顺序工作台均已完成；下一增量把用户确认后的 PDF 分段映射到既有任务/Checkpoint/队列，不复制调度器，仍不提前宣称已经可以导出。执行边界与实际差异见 [`PDF_MVP_PLAN.md`](PDF_MVP_PLAN.md)。
