...
- v0.5.1 的 DOCX 用户闭环已经接通安全导入、Checkpoint 恢复、有界翻译队列、安全重试、译文/双语重建、发布前取消和不覆盖原子导出；发布故障矩阵已覆盖权限、写入、同步和目标竞争，当前剩余真实 Word 视觉验收与发布收口。
...
1. 使用至少 5 份匿名真实 DOCX 完成 Word/LibreOffice 译文版与双语版逐页视觉验收；自动化语料不能替代该门槛。
2. 对齐普通 CI 与 Release 门禁，再统一提升 `0.5.1` 版本、更新发布文档并执行安装、升级、Updater 和资产完整性验收。
