...
2. PDF 已作为独立后续版本启动：受限文本层解析、4 份真实公开语料、阅读顺序工作台、统一任务确认和受限 PDF → 双模式 DOCX 导出核心均已完成；下一增量把 PDF 任务接入现有 Checkpoint/队列/取消/重试和真实模型请求，再开放界面导出动作，不复制调度器，仍不提前宣称已经发布。执行边界与实际差异见 [`PDF_MVP_PLAN.md`](PDF_MVP_PLAN.md)。
