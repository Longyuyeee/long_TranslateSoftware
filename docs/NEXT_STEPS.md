...
- Native Messaging v1、单 EXE Host、Windows 安装集成、桌面私有 IPC、配对授权和 `translate` / `cancel` 已完成；固定开发 ID 的 Manifest V3 扩展可由用户通过 `activeTab` 在当前页注入划词浮层，不声明持久网站权限。真实 Chrome/Edge 烟雾、商店 ID 和 `add_word` 尚未完成。
...
2. `translate` / `cancel` 与划词浮层已经完成代码接入；下一步独立实现 `add_word` 写入授权和收藏入口。
3. 完成 Chrome/Edge 安装、配对、划词、翻译、取消、收藏、升级和卸载真实烟雾后发布 `v0.5.0`。
...
- content script 与 Shadow DOM 隔离的划词浮层已完成，采用用户点击弹窗后单页注入、刷新失效的 `activeTab` 模式；下一步只实现 `add_word` 独立写入授权，随后完成 Chrome / Edge 全链路烟雾。PDF / Word 仍保持在 v0.5.1。
