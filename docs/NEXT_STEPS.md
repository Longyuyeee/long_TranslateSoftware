...
- 当前自动化基线：前端 41 个测试文件、181 项测试，Rust 64 项测试（含 2 项真实子进程测试）；生产 JavaScript 最大块为 234.55 KiB。
...
1. 拆分 `GeneralSettingsTab.tsx` 的 WebDAV、快捷键和维护区块，降低单页复杂度。
2. 把 `lib.rs` 剩余数据库/同步/导出逻辑迁入领域模块，并逐步统一 Tauri 错误。
