...
const zh = {
    ...
    documentWorkbenchDescription: "DOCX 预览已通过 WPS Office 与 LibreOffice 真实验收，Microsoft Word 正式兼容验收仍待完成；文本型 PDF 可复用同一任务队列并导出译文或双语 DOCX。",
    ...
    pdfPrivacyNotice: "PDF 在本机只读解析；只有你确认任务并点击开始后，分段正文才会发送给当前模型。源 PDF 不会被修改，输出始终写入新的 DOCX。",
    ...
    pdfPreviewScopeDescription: "PDF 首版按推断阅读顺序生成文本型 DOCX，不承诺像素级还原原版式；请在使用前检查最终分页与段落顺序。",
    ...
    "pdfWarning_complex-layout-review-required": "这一页包含分栏或类似表格的定位文本；翻译前必须核对阅读顺序。",
...
const en: TranslationCatalog = {
    ...
    documentWorkbenchDescription: "DOCX preview is validated with WPS Office and LibreOffice; formal Microsoft Word compatibility validation is still pending. Text-based PDFs reuse the same task queue and export translated or bilingual DOCX files.",
    ...
    pdfPrivacyNotice: "PDF text is inspected locally and read-only. Segments are sent to the current model only after you confirm the task and start it. The source PDF is never changed, and output is written to a new DOCX file.",
    ...
    pdfPreviewScopeDescription: "The first PDF version creates a text-first DOCX from inferred reading order and does not promise pixel-perfect layout. Check final pagination and paragraph order before use.",
    ...
    "pdfWarning_complex-layout-review-required": "This page contains columns or table-like positioned text. Verify its reading order before translation.",
