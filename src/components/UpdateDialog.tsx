...
type UpdateDialogContentProps = Omit<UpdateDialogProps, "open">;

function UpdateDialogContent({ version, notes, phase, progress, error, labels, onInstall, onClose }: UpdateDialogContentProps) {
    ...
    >
      ...
      >
        ...
        <footer className="flex justify-end gap-3 border-t border-black/5 bg-black/[0.02] px-7 py-5 dark:border-white/10 dark:bg-white/[0.025]">
          ...
          {!busy && (
            <button type="button" data-dialog-initial-focus onClick={onInstall} className="inline-flex items-center gap-2 rounded-xl bg-accent px-5 py-2.5 text-xs font-black text-white shadow-lg shadow-accent/20">
...
export default function UpdateDialog({ open, ...contentProps }: UpdateDialogProps) {
  return (
    <AnimatePresence>
      {open && (
        <UpdateDialogContent
